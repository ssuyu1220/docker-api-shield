...
# 1. 小工具：Log 發送器
# =======================
get_snippets(headers) := snippets if {
    b64_raw := object.get(headers, "x-crypto-json", "")
    b64_raw != ""
    json_str := base64.decode(b64_raw)
    payload := json.unmarshal(json_str)
    snippets := object.get(payload, "decoded_snippets", [])
} else := []
log_result(code, denys) := true if {
    ...
    # 只要有執行 http.send 就當作成功，不要因為 Log 伺服器掛掉而阻擋主要業務
