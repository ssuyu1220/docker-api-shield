...
type AnalyzeResponse struct {
	...
	Extracts Extracts `json:"extracts,omitempty"`
}
...
func buildSummary(s Signals) []string {
	var out []string
	if s.HasHostRootBind {
		out = append(out, "high-risk: host root (/) mounted")
	}
	if s.HasDockerSockBind {
		out = append(out, "high-risk: docker.sock mounted")
	}
	if s.HasObfuscatedBase64Blob {
		out = append(out, "high-risk: obfuscated base64 content detected")
	}
	if s.DecodeToShell {
		out = append(out, "high-risk: decode-to-shell chain")
	}
	return out
}
